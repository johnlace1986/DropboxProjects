...
namespace Utilities.Business
{
    ...
    public struct IntelligentString : IComparable, IEnumerable<Char>
    {
        ...
        {
            TimeSpan duration = new TimeSpan(milliseconds * TimeSpan.TicksPerMillisecond);
            return FormatDuration(duration, includeMilliseconds);
        }
        ...
        {
            String format = duration.Minutes.ToString("00") + ":" + duration.Seconds.ToString("00");

            if (includeMilliseconds)
                format += "." + duration.Milliseconds.ToString("000");

            if ((duration.Hours > 0) || (duration.TotalDays >= 1))
                format = duration.Hours.ToString("00") + ":" + format;

            if (duration.Days > 0)
                format = duration.Days.ToString("00") + ":" + format;
